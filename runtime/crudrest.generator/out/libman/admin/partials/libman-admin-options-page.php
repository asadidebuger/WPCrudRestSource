		...
		<v-card-title>
		  ...
		  <h2 class="pr-4 pl-4"><?php echo $this->title ?></h2>
			  ...
			  >
				...
				<span><?php echo __('Save Options', 'crwp') ?></span>
			  </v-btn>
		  <v-btn
				  class="mt-4 mb-4 mr-4 ml-4"
				  color="primary"
		  ...
		  >
			...
			<span><?php echo __('(Un)Check All', 'crwp') ?></span>
		  </v-btn>
		</v-card-title>
		<v-form
			ref="form"
		...
		>
		  <v-tabs vertical background-color="grey lighten-5">
			...
			<v-tab v-if="extendeds&&extendeds.length" >
			  <span ><?php echo __('Extended Web-Services','crwp') ?></span>
			</v-tab>
			  ...
			  <v-container >
				<v-row v-for="(role,r) in roles" v-id="r">
				  ...
				  <v-col cols="12" sm="1" v-for="(cap,c) in capsWS" v-id="c">
					<v-checkbox
							v-model="caps[r]['libman_ws_'+e+'_'+c]"
							:label="cap"
							color="primary"
							hide-details
					></v-checkbox>
			...
			<v-tab-item v-for="(page,p) in pages" v-id="'page-'+p">
			  <v-container>
				<v-row v-for="(role,r) in roles" v-id="r">
				  ...
				  <v-col cols="12" sm="1" v-for="(cap,c) in capsWS" v-id="c">
					<v-checkbox
							v-model="caps[r]['libman_page_'+p+'_'+c]"
							:label="cap"
							color="primary"
							hide-details
					></v-checkbox>
				  ...
				  <v-col cols="12" sm="1" v-for="(bulk,b) in page.listView.bulk" v-id="'bulk-'+b">
					<v-checkbox
							v-model="caps[r]['libman_bulk_'+p+'_'+b]"
							:label="bulk.title"
							color="primary"
							hide-details
					></v-checkbox>
			  ...
			  <v-container>
				<v-row v-for="(role,r) in roles" v-id="r">
				  ...
				  <v-col cols="12" sm="1" v-for="(extend,ex) in extendeds" v-id="ex">
					<v-checkbox
							v-model="caps[r]['libman_ws_'+extend]"
							:label="extend"
							color="primary"
							hide-details
					></v-checkbox>
	...
	const app = new Vue({
		...
		data() {
			return {
				...
				txt: {<?php echo "page:'" . __('Page') . "',ws:'" . __('Web-Service', 'crwp') . "', create:'" . __('Create') . "',update:'" . __('Update') . "',edit:'" . __('Edit') . "',back:'" . __('Back') . "',delete:'" . __('Remove') . "'" ?>},
				nonce: '<?php echo wp_create_nonce('wp_rest'); ?>',
				item: JSON.parse(`<?php echo get_option('libman_options', '{}') ?>`),
				caps: {},
				capsWS: {},
		...
		methods: {
			...
			checkAll(){
				...
				for(const e in entities){
					for(const r in roles){
						for(const c in capsWS){
							...
							caps[r]['libman_ws_'+e+'_'+c]=this.checkedAll;
						}
					}
				}
				for(const p in pages){
					...
					for(const r in roles){
						for(const c in capsWS){
							caps[r]['libman_page_'+p+'_'+c]=this.checkedAll;
						}
						for(const b in page.listView.bulk) {
							caps[r]['libman_bulk_' + p + '_' + b] = this.checkedAll;
						}
					}
				}
				...
				for(const ex in extendeds){
					...
					for(const r in roles){
							caps[r]['libman_ws_'+extend]=this.checkedAll;
					}
				}
