...
require_once microhrm_PATH.'includes/microhrm_field_type.php';
/** @var microhrm_Page $this */
...
foreach ($fieldsLayouts as $key=>$layout ){
  ...
  $fieldType = microhrm_field_type::get($field->type);
  if ($fieldType->url_js){
...
$can_view = current_user_can("microhrm_page_{$this->key}_view");
$can_edit = current_user_can("microhrm_page_{$this->key}_edit");
$can_delete = current_user_can("microhrm_page_{$this->key}_delete");
$can_create = current_user_can("microhrm_page_{$this->key}_create");
	...
	<v-container fluid>
	  <v-card v-if="item">
		<v-card-title>
		  ...
		  <small>{{modeTitle}}  <?php echo __($this->get_entity()->label,'microhrm'); ?></small> <h2 class="pr-4 pl-4">{{title}}</h2>
