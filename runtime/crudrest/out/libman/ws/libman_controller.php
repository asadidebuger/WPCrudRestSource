 ...
 */ 
class libman_controller extends WP_REST_Controller {
 ...
 */
  public function register_routes() {
	...
	$namespace = 'libman/v' . $version; //wp-json/libman/.$this->version;
	$base = $this->key;
  ...
  public function get_items( $request ) {
	include_once libman_PATH.'includes/libman_db.php';
	...
	if ($this->page_key) {
		$list=libman_Models::get_page($this->page_key)->listView;
	  	$where=$list->where;
	}
  ...
  public function delete_item( $request ) {
	...
	if (!$id) return new WP_Error('cant-delete', __('not permit!','libman'),  array( 'status' => 400 )  );
  ...
  public function permissions_check( $request ) {
	...
	if ($this->page_key) $key='libman_page_'.$this->page_key;
	else $key='libman_ws_'.$this->entity_key;
	...
	switch ($clbk){
	  ...
	  case 'delete_item':$key.='_delete';
		break;
	  case 'bulk_action': $key='libman_bulk_'.$this->page_key.'_'.$request->get_param('key');
		break;
	}
  ...
  public function bulk_action( $request ) {
	...
	return new WP_Error('not exist', __('Action not exist','libman'),  array( 'status' => 404 )  );
  }
  public function bulk_action_delete( $request,$items ) {
	...
	global $wpdb;
	$page=libman_Models::get_page($this->page_key);
	$ids=array();
  ...
  public function get_collection_params()
  {
	...
	$params["sort"] = array(
		'description' => __('List orderby','crwp'),
		'type' => 'string',
	...
	$params["select"] = array(
		'description' => __('Select fields for projection separate by `,`','crwp'),
		'type' => 'string',
