...
class libman_Models {
	...
	private function get_entity_publisher(){
		...
		// "publisher" Entity
		$publisher_title_script= 'item.title';
...
WHEREPUSH;
	...
	public function get_page_borrownotretuned(){
		$page=new libman_Page('borrownotretuned','borrow',__('Not returned','crwp'));
		$columns=array('borrowDate','returnDate','member','book',);
		$sortable=array('borrowDate','returnDate','member','book',);
		$searchable=array();
		$props=['createButton'=>true,'editButton'=>true,'searchBox'=>true,'detailButton'=>true,'deleteButton'=>true,'itemsPerPage'=>10];
...
WHEREPUSH;
	...
	public static function get_admin_menus():array{
		$menus=[
			'mylib'=>['title'=>__('My Library','crwp'), 'icon'=>'dashicons-book', 'sub'=>[
				...
				'borrow'=>['title'=>__('History','crwp'), 'pageTitle'=>__('Borrowed Books','crwp'),'page_key'=>'borrow'],
				'borrownotretuned'=>['title'=>__('Borrowed','crwp'), 'pageTitle'=>__('Not returned','crwp'),'page_key'=>'borrownotretuned'],
					],
				],
			];
		return $menus;
	...
	public static function get_pages(): array{
	  ...
	  self::get_page('borrownotretuned');
