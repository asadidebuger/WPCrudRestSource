...
<div id="libman-app" data-app class="v-application <?php echo is_rtl()?'rtl':'ltr'; ?>">
	...
	<v-container fluid>
		...
		<template v-slot:default="dialog">
		  <v-card>
			...
			<v-card-text color="error">
			  <div class="text-h5 pa-5"><?php echo __('Are you confirm this action?','crwp') ?></div>
	  ...
	  <v-card>
		  ...
		  <v-row  >
			<v-col md="4">
			  <v-btn v-if="access.create"
					  ...
					  @click="create"
			  >
				<?php echo __('Add New','crwp');?> <v-icon>mdi-file-plus </v-icon>
			  </v-btn>
			</v-col>
			<v-col md="5"></v-col>
			<v-col md="3" >
			  <v-overflow-btn
				  ...
				  :items="bulks"
				  label="<?php echo __('Select an action','crwp') ?>"
				  menu-props="top"
				  filled
				  dense >
		...
		<v-data-table
			...
			:items="items"
			:loading="loading"
			loading-text="<?php echo __('Loading...','crwp')?>"
			v-model="selected"
			:single-select="singleSelect"
			item-key="id"
			no-data-text="<?php echo __('Data not exist!','crwp')?>"
			no-results-text="<?php echo __('No data found!','crwp')?>"
			show-select
			class="elevation-1"
		...
		>
		  ...
		  <?php
		   $can_view=$this->get_list_view()->props["detailButton"] && current_user_can("libman_page_{$this->key}_view");
		   $can_edit=$this->get_list_view()->props["editButton"] && current_user_can("libman_page_{$this->key}_edit");
		   $can_delete=$this->get_list_view()->props["deleteButton"] && current_user_can("libman_page_{$this->key}_delete");
		   $can_create=$this->get_list_view()->props["createButton"] && current_user_can("libman_page_{$this->key}_create");
		...
		data() {
			return {
				...
				access:{
					...
					bulk:{<?php foreach ($this->get_list_view()->bulk as $key=>$bulk) echo "$key:".(current_user_can("libman_bulk_{$this->key}_{$key}")?'true':'false'); ?>}
				},
				itemsPerPage:1,
				currentPage:1,
				pagination:{},
