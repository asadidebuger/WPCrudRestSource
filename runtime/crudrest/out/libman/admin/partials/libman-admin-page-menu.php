...
<div id="app" data-app class="v-application <?php echo is_rtl()?'rtl':'ltr'; ?>">
  ...
  >
	...
	<h1>{{title}} <span :class="'dashicons-before '+menu.icon"></span>
	  <v-btn v-if="canConfigOptions" depressed @click="goToOptions"> <?php echo __('Options','crwp') ?> </v-btn>
	</h1>
	<v-container fluid>
	  <v-row>
...
<script>
	var app = new Vue({
		...
		methods: {
			...
			goToOptions(){
				window.location.href ='<?php echo get_site_url();?>/wp-admin/options-general.php?page=options_libman';
			}
		},
		computed: {
			urlMenus() {
